...
| Pack-structure classifier (`classify_pack` → typed `PackClass` / `PackKind` per the `stnsoft-vobov` rules — one pack per 2048-byte sector, one-or-two packets all of one kind (nav / video / MPEG-audio / private_stream_1 / padding), padding-only-second-packet, private_stream_2-only-inside-NAV; plus the `VOB_FILE_SPLIT_SECTORS` 0x7FFFF 1 GB file-split constant) | landed |
